...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggleSpamBtn.addEventListener('click', () => {
        ...
        if (state.isSpamming) {
            ...
            const delay = parseFloat(document.getElementById('message-delay').value) * 1000;
            const spamMode = document.getElementById('spam-mode').value;
            
            if (!message && !imageUrl && state.currentPlatform !== 'ngl') {
                addLog('Please enter a message or image URL', 'error', 'System');
                return;
            }
            
            if (state.currentPlatform === 'ngl' && !message) {
                addLog('Please enter a message for NGL', 'error', 'NGL');
                return;
            }
            ...
            } else if (state.currentPlatform === 'ngl') {
                startNGLSpam(message, count, delay, spamMode);
            }
        }
    });
});